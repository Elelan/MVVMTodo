...
class AddEditTaskViewModel @ViewModelInject constructor(
    ...
    private fun updateTask(task: Task) = viewModelScope.launch {
        taskDao.update(task)
        navigateBack(EDIT_TASK_RESULT_OK)
    }

    private fun createTask(task: Task) = viewModelScope.launch {
        taskDao.insert(task)
        navigateBack(ADD_TASK_RESULT_OK)
    }
    ...
    private fun navigateBack(result: Int) = viewModelScope.launch {
        addEditTaskEventChannel.send(AddEditTaskEvent.NavigateBackWithResult(result))
    }
